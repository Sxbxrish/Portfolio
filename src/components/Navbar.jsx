...
const Navbar = () => {
  ...
  const navLinks = ['Home', 'About', 'Services', 'Achievements', 'Internship', 'Project', 'Contact'];
        ...
        <div className="flex flex-col items-center gap-8">
          {navLinks.map((item) => (
            <a
              key={item}
              href={`#${item.toLowerCase()}`}
              className={`text-white text-3xl font-black uppercase tracking-widest hover:text-[#ccff00] transition-all duration-500 ${isMenuOpen ? 'delay-0' : ''}`}
              onClick={() => {
                setIsMenuOpen(false);
                const target = document.querySelector(`#${item.toLowerCase()}`);
                if (target) {
